package com.example.wirelesslocationstud.data
...
class WirelessApiIntegrationTest(private val context: Context) {
    ...
    private suspend fun testVerifyRectangularMap() {
        ...
        // Calculate expected dimensions from metadata
        val mapSize = RetrofitClient.wirelessMapApi.getMapSize()
        ...
        for (x in mapSize.minX..mapSize.maxX) {
            for (y in mapSize.minY..mapSize.maxY) {
                ...
                if (cell == null) {
                    missingCells++
                    if (missingCells <= 5) {
                        Log.w(TAG, "  Missing cell at (x=$x, y=$y)")
                    }
                } else if (cell.strength1 == 0 && cell.strength2 == 0 && cell.strength3 == 0) {
                    cellsWithZeroStrength++
                }
            }
        }

        if (missingCells > 0) {
            Log.w(TAG, "⚠ Found $missingCells missing cells (should have zero strengths)")
        }

        Log.d(TAG, "Cells with all zero strengths: $cellsWithZeroStrength")

        if (allCells.size == expectedCells) {
            Log.d(TAG, "✓ Map is complete rectangle with all cells present!")
        } else {
            Log.w(TAG, "⚠ Map is incomplete: ${allCells.size}/$expectedCells cells")
        }
    }
    ...
    private suspend fun testCacheStatus() {
        ...
        cellsByColumn.keys.sorted().forEach { x ->
            ...
            Log.d(TAG, "    x=$x: ${columnCells.size} cells ($nonZeroCells with signal)")
        }
